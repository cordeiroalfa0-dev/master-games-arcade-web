...
(() => {
  ...
  function createWebPlayer(romName, romUrl, message, overlay, parentUrl = "") {
    ...
    const onMessage = (event) => {
      ...
      if (event.data?.type === "mga-emulator-started") message.remove();
      if (event.data?.type === "mga-emulator-exit") closePlayer();
      if (event.data?.type === "mga-emulator-error") message.textContent = event.data.message || "Erro ao iniciar o jogo.";
    };
  ...
  window.MGA_WEB = Object.freeze({ launch: showWebPlayer, version: "1.5.0" });
  ...
  window.fetch = async function (input, init) {
    ...
    // A interface original ainda consulta esta rota de arte usando localhost:7777.
    // No modo WebAssembly não existe backend local. Retornamos 404 controlado
    // para permitir que o renderer use seu fallback sem gerar ERR_CONNECTION_REFUSED.
    if (path === "/api/art") {
      return jsonResponse({ ok: false, available: false, url: null }, 404);
    }
