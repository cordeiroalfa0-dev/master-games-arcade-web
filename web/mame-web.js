...
(() => {
  ...
  function createWebPlayer(romName, romUrl, message, overlay, biosUrl = "", biosName = "") {
    ...
    iframe.onload = () => { message.textContent = `INICIANDO ${cleanRomName(romName)}...`; };
  ...
  function showWebPlayer(romName) {
    return new Promise((resolve, reject) => {
      ...
      const overlay = document.createElement("div"); overlay.id = "mga-web-player"; overlay.style.cssText = "position:fixed;inset:0;z-index:2147483647;background:#000;overflow:hidden;font-family:Arial,sans-serif;";
      const area = document.createElement("div"); area.id = "mga-web-player-area"; area.style.cssText = "position:absolute;inset:0;background:#000;";
      const bar = document.createElement("div"); bar.style.cssText = "position:absolute;top:0;left:0;right:0;height:44px;z-index:20;display:flex;align-items:center;justify-content:space-between;padding:0 12px 0 14px;box-sizing:border-box;background:linear-gradient(180deg,#08000f 0%,rgba(8,0,15,.78) 72%,transparent 100%);color:#fff;pointer-events:none;";
      const brand = document.createElement("strong"); brand.textContent = "MASTER GAMES ARCADE"; brand.style.cssText = "font-family:monospace;color:#00e5ff;letter-spacing:1px;font-size:12px;text-shadow:0 0 8px #00e5ff;"; bar.appendChild(brand);
