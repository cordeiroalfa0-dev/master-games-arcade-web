...
const gameManager = fs.readFileSync(path.join(root, 'web/emulatorjs-data/src/GameManager.js'), 'utf8');
...
assert.match(player, /mgaAudioResumePending/);
assert.match(gameManager, /audio_latency = 384/);
assert.match(gameManager, /audio_rate_control_delta = 0\.01/);
assert.match(gameManager, /audio_max_timing_skew = 0\.10/);
...
assert.match(player, /getCoreOptions/);
assert.match(player, /setVariable/);
assert.match(player, /applyDifficultyToCore/);
assert.match(player, /visual\.aspect='stretch'/);
assert.doesNotMatch(player, /data-setting="aspect"/);
assert.doesNotMatch(player, /btn-rotate/);
assert.match(player, /data-setting="palette"/);
assert.match(player, /data-setting="shader"/);
assert.match(player, /Segundo controle USB/);
assert.match(player, /loading-art/);
...
assert.match(launcher, /rel="preconnect" href="https:\/\/cdn\.jsdelivr\.net"/);
assert.match(player, /fetch\(window\.EJS_biosUrl,\{method:'HEAD'\}\)\.then/);
assert.match(player, /orientationResizeFrame/);
assert.match(player, /scheduleHideEmulatorUi/);
...
assert.ok(fs.existsSync(path.join(root, 'assets/game-loading-art.png')), 'arte de carregamento ausente');
assert.ok(fs.existsSync(path.join(root, 'assets/game-loading-art-portrait.png')), 'arte vertical de carregamento ausente');
assert.ok(fs.existsSync(path.join(root, 'assets/game-loading-art-wide.png')), 'arte horizontal larga de carregamento ausente');
